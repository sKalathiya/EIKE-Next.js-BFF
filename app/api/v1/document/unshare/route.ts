...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${GATEWAY_URL}/api/v1/document/unshare/${encodeURIComponent(id)}`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      isRecord(payload) ? payload : { message: "Could not unshare the file." },
      { status: response.status },
    );
  } catch {
    return NextResponse.json({ message: "Failed to unshare the file." }, { status: 500 });
  }
}
